...
down_revision: Union[str, None] = '004_pgvector_document_chunks'
...
def upgrade() -> None:
    ...
    op.create_table('lesson_readings',
        ...
        sa.ForeignKeyConstraint(['lesson_id'], ['generated_content.id'], ),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_lesson_readings_user_id'), 'lesson_readings', ['user_id'], unique=False)
    op.create_index(op.f('ix_lesson_readings_lesson_id'), 'lesson_readings', ['lesson_id'], unique=False)
