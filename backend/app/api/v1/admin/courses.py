...
async def create_course(
    ...
    logger.info("Course created", course_id=str(course.id), slug=course.slug, admin=user.email)
...
async def publish_course(
    ...
    await db.commit()
    await db.refresh(course)
    logger.info("Course published", course_id=str(course_id), admin=user.email)
    return CourseResponse.model_validate(course)
