...
async def get_image_data(
    ...
    """Proxy image binary from MinIO — fetches internally and returns bytes to browser."""
    result = await db.execute(select(SourceImage).where(SourceImage.id == image_id))
    img = result.scalar_one_or_none()
    if img is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Image not found")
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY, detail="Failed to fetch image from storage"
        )
