...
{{
  ...
  "time_limit_minutes": {max(SettingsCache.instance().get("quiz__time_limit_min_minutes", 10), num_questions * SettingsCache.instance().get("quiz__time_limit_per_question_minutes", 1.5))},
  "passing_score": {SettingsCache.instance().get("quiz__passing_score", 80.0)}
}}
...
Generate the quiz now, ensuring all questions are relevant to public health practice in West Africa."""
    ...
    def _validate_and_normalize_quiz(
        ...
        try:
            ...
            _passing = SettingsCache.instance().get("quiz__passing_score", 80.0)
            quiz_data.setdefault(
                "time_limit_minutes",
                max(
                    SettingsCache.instance().get("quiz__time_limit_min_minutes", 10),
                    len(questions)
                    * SettingsCache.instance().get("quiz__time_limit_per_question_minutes", 1.5),
                ),
            )
