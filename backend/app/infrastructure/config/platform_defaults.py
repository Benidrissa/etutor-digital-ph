...
SETTING_DEFINITIONS: list[SettingDef] = [
    ...
    SettingDef(
        ...
        "Compact after this many messages. Bumped 50→80 (#1995) — caching keeps cost flat.",
        {"min": 10, "max": 400},
    ),
    SettingDef(
        ...
        "Verbatim messages kept post-compact. Bumped 20→40 (#1995).",
        {"min": 2, "max": 100},
    ),
    SettingDef(
    ...
    SettingDef(
        ...
        10000,
        "integer",
        "Module-detail per-item excerpt chars",
        "Per-unit content length when a unit's content is generated and cached.",
        {"min": 500, "max": 20000},
    ),
    SettingDef(
        "tutor-course-block-char-limit",
        "tutor",
        20000,
        "integer",
        "Course-block char limit",
        "Hard cap on the course-level block (syllabus + resources + cross-module map).",
        {"min": 2000, "max": 40000},
    ),
    SettingDef(
        "tutor-context-caching-enabled",
        "tutor",
        True,
        "boolean",
        "Tutor prompt caching",
        "Use Anthropic prompt caching for the tutor system prompt. Set to false to disable.",
        {},
    ),
    SettingDef(
        "tutor-include-quiz-answers",
        "tutor",
        True,
        "boolean",
        "Tutor: include quiz answers",
        "Include the correct answer + explanation for every quiz question in the module block.",
        {},
    ),
    SettingDef(
        "tutor-include-cross-module-map",
        "tutor",
        True,
        "boolean",
        "Tutor: include cross-module map",
        "Render every other module's title + generated/pending status in the course block.",
        {},
    ),
    SettingDef(
    ...
    # ── Payment providers (API checkout) ───────────────────
    # Enable flags are public (drive the frontend method selector); all keys
    # are is_sensitive so get_all_public never leaks them to the browser.
    SettingDef(
        "payments-orange-money-enabled",
        "payments",
        False,
        "boolean",
        "Enable Orange Money",
        "Show Orange Money in the checkout selector (uses the Web Payment API).",
    ),
    SettingDef(
        "payments-wave-enabled",
        "payments",
        False,
        "boolean",
        "Enable Wave",
        "Show Wave in the checkout selector (uses the Wave Checkout API).",
    ),
    SettingDef(
        "payments-paystack-enabled",
        "payments",
        False,
        "boolean",
        "Enable Paystack (card)",
        "Show card/Paystack in the checkout selector.",
    ),
    SettingDef(
        ...
        "string",
        "Paystack secret key",
        "Paystack secret key (sk_…). Used for API calls and webhook HMAC. Server-side only.",
        is_sensitive=True,
    ),
    SettingDef(
        "payments-paystack-currency",
        "payments",
        "XOF",
        "string",
        "Paystack currency",
        "Settlement currency for Paystack charges (XOF, NGN, GHS, USD).",
    ),
    SettingDef(
        "payments-wave-api-key",
        "payments",
        "",
        "string",
        "Wave API key",
        "Wave Business API key for creating checkout sessions. Server-side only.",
        is_sensitive=True,
    ),
    SettingDef(
        "payments-wave-webhook-secret",
        "payments",
        "",
        "string",
        "Wave webhook secret",
        "Secret used to verify the Wave-Signature header on webhooks.",
        is_sensitive=True,
    ),
    SettingDef(
        "payments-orange-client-id",
        "payments",
        "",
        "string",
        "Orange API client ID",
        "Orange Developer OAuth consumer key. Server-side only.",
        is_sensitive=True,
    ),
    SettingDef(
        "payments-orange-client-secret",
        "payments",
        "",
        "string",
        "Orange API client secret",
        "Orange Developer OAuth consumer secret. Server-side only.",
        is_sensitive=True,
    ),
    SettingDef(
        "payments-orange-merchant-key",
        "payments",
        "",
        "string",
        "Orange merchant key",
        "Orange Money Web Payment merchant key. Server-side only.",
        is_sensitive=True,
    ),
    SettingDef(
        "payments-orange-country",
        "payments",
        "civ",
        "string",
        "Orange Money country code",
        "Country segment for the Web Payment API path (e.g. civ, sen, mli).",
    ),
    # ── AI Prompts ─────────────────────────────────────────
    SettingDef(
        ...
        (
            ...
            "- Language: {language}\n\n"
            "STRICT TOPIC BINDING:\n"
            "The lesson MUST address EXCLUSIVELY the topic of the unit named above"
            " ({unit_title}). Do NOT cover other units of the module — each unit has"
            " its own dedicated lesson. The unit's specific topic is also restated in"
            " the user message.\n\n"
    ...
    SettingDef(
        ...
        (
            ...
            "Section 2 — real_data (tables, structured lists, or other "
            "appropriate evidence formats):\n"
            "   - Quantitative data where available: key figures, "
            "measurable indicators\n"
            "   - Temporal or process data: event timeline, decision "
            "sequence, or workflow\n"
            "   - Contextual data: geographic, organizational, or "
            "stakeholder distribution\n"
            "   - Sources: professional organizations, institutional "
            "reports, sector data, or authoritative references\n\n"
    ...
    SettingDef(
        ...
        (
            ...
            "CRITICAL REQUIREMENTS:\n"
            "- Base content ONLY on provided documents — do not invent information\n"
            "- Do NOT include source citations in the lesson text"
            " (no brackets like [Author Ch.X, p.Y])"
