...
export interface UserGroupDetailResponse extends UserGroupResponse {
...
export async function getAdminGroup(id: string): Promise<UserGroupDetailResponse> {
  const { authClient } = await import("./auth");
  return authClient.authenticatedFetch<UserGroupDetailResponse>(`/api/v1/admin/groups/${id}`);
}
...
export async function addGroupMember(
  ...
  const { authClient } = await import("./auth");
  return authClient.authenticatedFetch<UserGroupMember>(
